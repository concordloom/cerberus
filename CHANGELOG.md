...
Generated by [semantic-release](https://semantic-release.gitbook.io) from the
commit messages on `main`. The `1.0.0` entry below was written by hand: it was
generated from commits predating the English-only policy, and an English project
with a Russian changelog is not a record anyone can read.
